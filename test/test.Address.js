...
describe('Address', function() {
  ...
  it('should be able to create instance', function() {
    var a = new Address('DAoDKB7WMU4Xw1B3ThYstgmUzAYzzHQi6B');
    should.exist(a);
  });
  it('should be able to transform to string', function() {
    var a = new Address('DSVERSgwECfKGxLq6XHi6u5zUq7XhGNC9M');
    a.toString.bind(a).should.not.throw();
    a.toString().should.equal('DSVERSgwECfKGxLq6XHi6u5zUq7XhGNC9M');
  });
  var data = [
    ['DLarstbuuFbTj2nKpNjRYbECj9joA1dp5j', true],
    ['11111111111111111111111111122222234', false], // totally invalid
    // ['A9GnWZNerHENehGK4gfzTP7TrFD9fa78FJ', true], // FIXME: make multisig for mainnet work
    // ['9znypFJ4XkURxzBQJ4DVJN7tatGdHZampp', true], // FIXME: make multisig for testnet work
    ['1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNb', false], //bad checksum ... thanks @wtogami
    ['DLeFeKNcdoWwMVd8zriQtKn2RatwiJmUeh', true],
    ...
    ['DRbUWAc4NHDYeRuYsJADcg3Qn1jKHC9XxZ', true], // valid base58, valid length, valid network
  ];
  ...
  it('should be able to detect network from an address', function() {
    ...
    a = new Address('D9QQ4PiHhYEDChoirVaxaqhscSGAdjGj92');
    a.network().name.should.equal('livenet');
    //p2sh
    // a = new Address('3QRhucKtEn5P9i7YPxzXCqBtPJTPbRFycn'); // FIXME: find a p2sh dogecoin mainnet address and test it
    // a.network().name.should.equal('livenet');

    //testnet
    a = new Address('nitBRo7CF1G1BvUwcKuMoJRfXFvxZCjeeo');
    a.network().name.should.equal('testnet');
    a = new Address('nesAdTvX2gqz8iUe5FtyMNzmrAX1ffU6jB');
    a.network().name.should.equal('testnet');

    //p2sh
    // a = new Address('2NBSBcf2KfjPEEqVusmrWdmUeNHRiUTS3Li'); // FIXME: find a p2sh dogecoin testnet address and test it
    // a.network().name.should.equal('testnet');
  });
  it('#isScript should work', function() {
    ...
    // pubKeyHash livenet
    new Address('DAoDKB7WMU4Xw1B3ThYstgmUzAYzzHQi6B').isScript().should.equal(false);
    // script livenet
    new Address('3QRhucKtEn5P9i7YPxzXCqBtPJTPbRFycn').isScript().should.equal(true);
    // pubKeyHash testnet
    new Address('nitBRo7CF1G1BvUwcKuMoJRfXFvxZCjeeo').isScript().should.equal(false);
    // script testnet
  ...
  describe('#Address constructor', function() {
    it('should produce a valid address from a hash', function() {
      ...
      var addr = new bitcore.Address(30, hash);
  ...
  describe('#fromPubKey', function() {
    it('should make pubkeyhash address from an uncompressed public key', function() {
      var pubkey = new Buffer('04da9420ded1d3aa814fd5eff44bb05a7d9c10fc42d8443c28abe7cba320190a3d39cf61032d4249dfff6e387aad27a0aefafe7c8e2a1fa55a8097bd77c115949a', 'hex');
      var hash = bitcore.util.sha256ripe160(pubkey);
      var addr = new Address(30, hash);
